...
meta <- read.csv("DESTv2_data_paper/misc/Inversions/data/dest_v2.samps_25Feb2023.csv", row.names = 1)
nhm_inversion <- read.delim("DESTv2_data_paper/misc/Inversions/results/PoolSNP_nhm_inversion.af", row.names = 1)
for (col in colnames(nhm_inversion)) {
  nhm_inversion[[col]] <- as.numeric(paste(nhm_inversion[[col]]))
}

dir.create("DESTv2_data_paper/misc/Inversions/results/Maps")
...
for (i in inversions) {
  print(i)
  pdf(paste("DESTv2_data_paper/misc/Inversions/results/Maps/Global_Latitude_", i, ".pdf", sep = ""), width = 12, height = 8)
  print(ggplot(data2, aes(x = lat, y = data2[[i]], col = continent)) +
    ...
    guides(fill = FALSE) +
    ggtitle(paste("Continents - ", i, sep = "")) +
...
pdf("DESTv2_data_paper/misc/Inversions/results/Maps/Global_Longitude.pdf", width = 12, height = 8)
# par(mfrow=c(2,3))
for (i in inversions) {
  print(i)
  pdf(paste("DESTv2_data_paper/misc/Inversions/results/Maps/Global_Longitude_", i, ".pdf", sep = ""), width = 12, height = 8)
...
ggplot(N12t18, aes(x = lat, y = In.3R.Payne, col = year)) +
  geom_point() +
  geom_smooth(method = "glm", method.args = list(family = "binomial"), formula = y ~ x, alpha = .15, aes(fill = year)) +
  guides(fill = FALSE) +
  ggtitle("North American Samples: 2012-2018") +
  theme(plot.title = element_text(hjust = 0.5))

E14t21 <- subset(Europe, year >= 2014 & year <= 2021)
E14t21$year <- as.factor(paste(E14t21$year))
ggplot(E14t21, aes(x = lat, y = In.3R.Payne, col = year)) +
  geom_point() +
  geom_smooth(method = "glm", method.args = list(family = "binomial"), alpha = .15, aes(fill = year)) +
  guides(fill = FALSE) +
  ggtitle("European Samples: 2014-2021") +
  theme(plot.title = element_text(hjust = 0.5))
