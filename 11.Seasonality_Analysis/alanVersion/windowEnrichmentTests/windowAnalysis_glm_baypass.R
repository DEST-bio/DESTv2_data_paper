...
### run windows
  ...
  win.out <- foreach(win.i=1:dim(wins)[1], .errorhandling = "remove", .combine = "rbind"  )%dopar%{
    ...
    foreach(pr.i =c(1e-4, 1e-3, 1e-2, 0.05), .combine="rbind")%do%{
...
### plot
  ...
  geom_vline(data=data.table(chr="3R", pos_mean=13223000), aes(xintercept=pos_mean), color="green") +
    geom_line(data=win.out,aes(x=pos_mean, y=-log10(c2.rnp.binom.p)), color="red") +
    facet_grid(~chr)

    ggplot(data=win.out) +
   geom_vline(data=data.table(chr="3R", pos_mean=13223000), aes(xintercept=pos_mean), color="green") +
     geom_line(data=win.out,aes(x=pos_mean, y=-log10(xtx_glm.p)), color="red") +
     facet_grid(~chr)
