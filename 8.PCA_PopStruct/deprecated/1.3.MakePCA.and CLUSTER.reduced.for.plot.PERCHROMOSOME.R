...
CHR="2R"
snps.dt <- snps.dt[nAlleles==2][missing < 0.1][chr %in% c(CHR)][pos < 14000000]
...
set.seed(12345)
seqSetFilter(genofile, sample.id=samps$sampleId, 
             variant.id=snps.dt[sample(dim(snps.dt)[1], 10000 )]$variant.id)
...
save(pca.object.plot, file = 
       paste(CHR,"pca.object.plot.Rdata", sep = ".")
       )
...
ggsave(PCA12.lat, file = paste(CHR,"PCA12.lat.pdf", sep = "."), w = 5, h =4)
...
ggsave(PCA13.lat, file = paste(CHR,"PCA13.lat.pdf", sep = "."), w = 5, h =4)
...
set.seed(123)
km.res <- kmeans(dat.cluster, 8, nstart = 25)
# 3. Visualize
pdf("clust.viz.pdf")
...
base_world + 
  ...
  ggtitle(paste(CHR, "No Inversion" ))  +
  scale_fill_brewer(palette = "Set1") -> cluster.plot

ggsave(cluster.plot, file = paste(CHR,"cluster.plot.pdf",sep = "."), w = 7, h = 3.5)
