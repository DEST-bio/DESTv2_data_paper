...
divm[divm$continentf=="Oceania",]$continentf<-"Australia"
divm[divm$continentf=="North_America",]$continentf<-"North America"
divm[divm$continentf=="South_America",]$continentf<-"South America"

new_pi <- fread("Xwide_Autosomes_PiEstimates_SexRatio")
new_pi2<-merge(new_pi,divm[,c("sampleId", "continentf")],by.x="SampleID",by.y="sampleId")

d<-ggplot(data=new_pi2, aes(x=continentf, y=A_Pimed_10, color=continentf,fill=continentf))+
#   #geom_boxplot(data=pop_genome_stats_pi_m,aes(x=Continent,y=value,fill=Continent), show.legend = F)+
  ...
  scale_color_manual(values=c("#7f2831","#a3492d","#a06e3e","#9b8a5a","#8e8e6b","#00687f","#6c8c67","#407262","#1a4863")) + ylim(c(min(new_pi2$A_Pimed_10),max(new_pi2$A_Pimed_10)))
d

d2<-ggplot(data=divm, aes(x=continentf, y=Pi, color=continentf,fill=continentf))+
  #   #geom_boxplot(data=pop_genome_stats_pi_m,aes(x=Continent,y=value,fill=Continent), show.legend = F)+
  geom_boxjitter( jitter.shape = 21,  
                  outlier.color = NA, errorbar.draw = TRUE, show.legend = F)+
  ylab(expression("Nucleotide diversity ("~paste(pi)~")"))+
...
line_color <- "grey"
line_color2 <- "black"
ci_color <- "orange"  # Set the color for the confidence interval
...
e <- ggplot(datalong, aes(x = pos, y = value)) +
  annotate("rect", xmin = 2225744, xmax = 13154180, ymin = -Inf, ymax = Inf, fill="lightblue",alpha=0.2) +
...
library(cowplot)
ggsave("figure1_v3.pdf",plot=plot_grid(a,b,align="ltrb",labels=c("A","B"),ncol=1,rel_heights = c(1,0.6)), height = 8, width = 8)

figure2_new <-plot_grid(plot_grid(c,d,ncol=2,align="tb",labels=c("A", "B")),e,labels=c("","C"),ncol=1,align="lrtb")
ggsave("figure2_v4.pdf",plot=figure2_new, height = 8, width = 8)
